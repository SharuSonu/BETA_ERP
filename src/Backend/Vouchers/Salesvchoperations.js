...
async function createSalesVoucher(reqBody) {
    ...
    try {
        ...
        const salesLedgerValue = '';//salesLedger.value;
    ...
    try {
        ...
        await connection.query(`
            ...
            BEGIN
                ...
                IF EXISTS (SELECT 1 FROM stock_summary WHERE stockItemName = NEW.itemName) THEN
                    SELECT 
                ...
                INSERT INTO stock_summary (stockItemName, periodFrom, periodTo, outwardsQuantity, outwardsValue, closingBalanceQuantity, closingBalanceValue)
                VALUES (NEW.itemName, NEW.voucherDate, NEW.voucherDate, NEW.quantity, NEW.amount, closingBalanceQty - NEW.quantity, closingBalanceAmt - NEW.amount)
        ...
        await connection.query(`
            ...
            BEGIN
                ...
                -- Fetch the current closing balances
                IF EXISTS (SELECT 1 FROM stock_summary WHERE stockItemName = OLD.itemName) THEN
                    SELECT 
                        IFNULL(closingBalanceQuantity, 0), IFNULL(closingBalanceValue, 0)
                    INTO 
                        closingBalanceQty, closingBalanceAmt
                ...
                SET
                    outwardsQuantity = outwardsQuantity - OLD.quantity,
                    outwardsValue = outwardsValue - OLD.amount,
                    closingBalanceQuantity = closingBalanceQty + OLD.quantity,
                    closingBalanceValue = closingBalanceAmt + OLD.amount
                WHERE 
                    stockItemName = OLD.itemName;
            END
        `);
